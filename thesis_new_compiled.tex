...
The architecture is a contract between three files. \texttt{prepare.py} holds the data and defines the score; nobody edits it, so every experiment is scored identically. \texttt{train.py} holds the model and the training loop and is edited only by the agent; \texttt{program.md} holds the instructions and is edited only by the human.

The experiment cycle exists only as English prose in \texttt{program.md}, and the model itself is the execution layer. \texttt{program.md} is therefore not configuration supplied to a controller; it \emph{is} the controller.

\subsection{Motivation and Research Objective}

An instruction file settles five matters at once: what counts as success, how widely the agent may search, when it must stop, how it must report, and where to concentrate effort. Two differently worded files therefore differ in all five at once, so no effect can be attributed to any single matter. Separating them requires a file in which one matter changes while the rest stay fixed, which is to treat the file as a configuration vector rather than as prose. Five components of \texttt{program.md} are each written at two levels, and all $2^5 = 32$ combinations are generated and executed. The aim is not to propose a better autoresearch loop, but to establish what an existing one responds to.

Every comparison is between two configurations of the same system; no human baseline is used. The research questions are:
...
Table~\ref{tab:axes} lists the levels. A production \texttt{program.md} hardcodes the baseline to be exceeded, specifies how failures are handled, fixes a reporting discipline and imposes a parsimony rule \cite{Tuychiev2026guide}. The five components in the table are drawn from those categories. One of them matters more than it looks: the loop carries no state except its own transcript. Before proposing the next experiment it can read only what it wrote in the hypothesis and interpretation fields of earlier turns. The reporting style $O$ therefore controls how much of its own reasoning the loop can later read.
...
The \emph{cost to best} is the number of executor calls up to and including the one that produced the run's highest score, taking the earliest such call on ties.

\paragraph{Measurement window.} The stopping rule determines how many experiments a run performs, and every metric above aggregates over those experiments. All four are therefore computed over the first three experiments, the number performed at the fixed-budget level and hence available in every run.
...
\label{eq:regression}
\end{equation}
where $D_j$ indicates that the run was made on dataset $j$, with the first dataset as reference. The $\gamma_j$ are dataset fixed effects. They absorb the inherent difficulty of each task, its headroom above $a_0(d)$ and its response to the whitelist. Here $\beta_0$ is the mean across configurations on the reference dataset, $\beta_i$ the effect of component $i$, and $\varepsilon$ run-to-run error. $2\beta_i$ is the average change in $Y$ produced by switching component $i$, and $2\hat\beta_i$ is reported throughout. Because all thirty-two configurations are executed, the interaction model is also identified,
\begin{equation}
...
In an autoresearch loop, \texttt{program.md} does what controller code does in conventional software, yet cannot be read like code. This thesis has treated it as a configuration vector. Five components were written at two levels each, all $2^5=32$ configurations were generated and executed against a real executor on two datasets with twenty replicates each, and the behavior of the $1{,}280$ runs was reduced to four metrics and regressed on an orthogonal $\pm1$ coding of the components. Because the configurations were constructed rather than collected, the coefficients admit a causal reading. Because the factorial is complete, the independence of the components could be tested rather than assumed.
