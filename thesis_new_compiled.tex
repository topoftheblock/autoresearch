...
The experiment cycle exists only as English prose within \texttt{program.md}, and the model itself is the execution layer. \texttt{program.md} is therefore not configuration supplied to a controller; it \emph{is} the controller.

\subsection{Motivation and Research Objective}

An instruction file settles several matters simultaneously: what counts as success, how widely the agent may search, when it must stop, how it must report, and where effort should be concentrated. Two differently worded files therefore differ in all five at once, and no individual effect can be attributed to any one of them. Separating them requires a file in which one matter can be changed while the rest are held fixed, which is to treat it not as prose but as a configuration vector. Five components of \texttt{program.md} are accordingly each written at two levels, and all $2^5 = 32$ combinations are generated and executed. The aim is not to propose a superior autoresearch loop, but to establish what an existing one is responding to.

Every comparison is between two configurations of the same system; no human baseline is invoked. The following research questions are raised:
...
\paragraph{Compliance.} Table~\ref{tab:interactions} also reports the share of runs whose behavior matched the instruction given, so that a component without effect can be told apart from one the model disregarded. $B$ is checked by the number of model families a run proposes, $S$ by the number of experiments it performs and $E$ by whether its second proposal refines its first; $O$ constrains wording rather than action, so it is given the mean characters of model-written text per step instead; $M$ leaves no behavioral trace, because the executor computes the score whatever the instruction says. The fixed budget is followed in $89\%$ of runs and the adaptive rule in only $20\%$, so the stopping-rule effects measure the effect of offering an adaptive budget rather than of a loop that reliably stops on diminishing returns, and every estimate in Table~\ref{tab:main} is an intention-to-treat estimate.
