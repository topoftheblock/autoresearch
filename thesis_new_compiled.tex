...
Establishing how is an empirical problem. This project treats the instruction file not as prose but as a configuration vector. Five components of \texttt{program.md} are each written at two levels, and all $2^5 = 32$ combinations are generated and executed against a real executor, which separates the effect of each component instead of assuming it. The contribution is a method rather than an architecture: the aim is not to propose a superior autoresearch loop, but to establish what an existing one is responding to.
