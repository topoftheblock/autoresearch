...
\texttt{train.py} holds the model and is the only file the agent edits, and
\texttt{program.md} holds the instructions. The loop studied here narrows the search
space from arbitrary code to a hyperparameter space (Section~\ref{sec:loop}), and that
narrowing removes one of the three files: the agent proposes a model family and
hyperparameter values as JSON and never edits code, so there is no agent-owned
\texttt{train.py}. What remains is \texttt{program.md}, reproduced in
Appendix~\ref{app:program}, and a harness that plays the part of \texttt{prepare.py}
and of the training loop together. That harness is two modules, given in
Appendices~\ref{app:executor} and~\ref{app:data}: the executor fits and scores a
proposal, and the dataset registry fixes the partition. Neither is reachable by the
agent, which is what makes every number in a transcript a measurement rather than a
claim.
...
The one part of the loop that is not a language-model call. It validates a proposal
against the whitelist, fits the estimator and returns the same record every time,
whatever the configuration's evaluation paragraph asks for.
...
\section{The dataset registry}
\label{app:data}

One stratified $70/30$ partition per task, fixed with the same seed for every run, so
the split is a constant of the study rather than a source of variation.
