...
The improvement rate is the proportion of executed trials whose score is strictly higher than the best score observed earlier in the same run. A run of eight trials containing a single improvement approaches random search, whereas a high rate constitutes evidence that the interpret step is informing the next proposal.
...
One qualification is required. The evaluation-criterion component at level 1 names cross-validated accuracy, which is the scale on which the gain over default is computed, so that part of any effect exhibited by that component reflects agreement between the treatment and the instrument. For the first dataset the gain on held-out validation accuracy is reported as a robustness check, and an effect is claimed for that component only where both scales agree. The second dataset admits no such check, for the reason given in Section~\ref{sec:setup}.
