...
\begin{abstract}
An autoresearch loop is one in which a language model proposes, executes and interprets experiments without a human in the loop. In the form made popular by Karpathy's \texttt{autoresearch}, that procedure is written in natural language in a single  file, \texttt{program.md}. This thesis treats the instruction file as a configuration vector. Five components of \texttt{program.md} are each written at two levels. All $2^5 = 32$ combinations are generated and executed on two classification tasks with twenty replications each. This resulted in $1{,}280$ runs. The components determine the process of the search rather than its outcome.  \end{abstract}
...
In an autoresearch loop, a language model performs the experimentation. It is given a goal, a body of code it can edit. It then works unattended. It proposes a change, executes it, reads the outcome, decides what to try next, and repeats until the budget is exhausted. Karpathy's \texttt{autoresearch} \cite{Karpathy2026autoresearch} fixed the current meaning of the term. It appeared in March 2026 and has since been forked several thousand times.

The architecture is comprised of three files. \texttt{prepare.py} holds the data and defines the score. Nobody can edit it, so every experiment is scored identically. \texttt{train.py} holds the model and the training loop and is edited only by the agent. \texttt{program.md} holds the instructions and is crafted by the human.

The experiment cycle exists only as natural language in \texttt{program.md}. The model is the execution layer, and \texttt{program.md} is therefore is the controller.

\subsection{Motivation and Research Objective}

An instruction file settles five matters at once: what counts as success, how widely the agent is allowed to search, when it must stop, how it must report, and where to concentrate its effort. If two differently worded files therefore differ in all five at once, then no effect can be attributed to any single matter. Separating the effects requires a \texttt{program.md} file in which one configuration changes while the rest stay fixed. This is to treat the file as a configuration vector. Five components of \texttt{program.md} are each written at two levels. All $2^5 = 32$ combinations are generated and executed. The aim of the research project is not to propose a better autoresearch loop, but instead to establish what an existing one responds to.
...
A single reference quantity is computed per dataset before any run. The quantity $a_0(d)$ is the accuracy of a default-hyperparameter fit. On both datasets the better family is the random forest.
\subsection{Design and Budget}

The design is the complete $2^5$ factorial for each dataset and replicated $R$ times: $N = 2^5 \cdot D \cdot R$ runs. With $D=2$ and $R=20$ that is $N=1{,}280$ runs. Of them, $1{,}278$ ended on the model's own stop decision and two on the upper-bound of eight experiments. The study did about $9{,}900$ model calls and $4{,}768$ estimator fits, the fits took $74$ minutes of executor time in total.
