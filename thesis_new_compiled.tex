...
Five of the sixty-four cells are incomplete, all of them on Wine and all at $M{=}1$,
$B{=}1$: four contain no runs and one contains five of twenty replicates, an API request
quota having been reached before they executed. The design is therefore not exactly
balanced in the sense described in Section~\ref{sec:setup}. Because the missing cells are
systematic rather than randomly distributed, the estimates for $M$ and $B$ are the least
reliable. All remaining cells contain the full twenty replicates.
